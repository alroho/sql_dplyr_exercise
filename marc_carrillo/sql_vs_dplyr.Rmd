...
```{r}
match_db <- tbl(sql_con, "Match")
grouped_by_country <- group_by(match_db, country_id)
arrange(summarize(grouped_by_country, count = n()), desc(count))
```
...
* List 5 players and their highest rating that had the lowest rating ever.
```{sql connection=sql_con}
SELECT Player.player_name, pa.min_rating 
FROM Player
LEFT JOIN (
  SELECT player_api_id, MIN(overall_rating) AS min_rating
  FROM Player_Attributes
  GROUP BY player_api_id) pa
ON Player.player_api_id = pa.player_api_id
ORDER BY pa.min_rating ASC
LIMIT 10
```
...
player_db %>%
  left_join(
    {player_attributes_db %>% 
      group_by(player_api_id) %>%
      summarise(min_rating=min(overall_rating, na.rm=TRUE))}, 
    by='player_api_id') %>%
  arrange(min_rating) %>%
  select(player_name, min_rating) %>%
  head(10)
```
...
player_attributes_db %>%
  ...
  arrange(min_rating) %>%
  left_join(player_db, by='player_api_id') %>%
  select(player_name, min_rating) %>%
  head(10)
```
...
FROM Team t
WHERE t.team_api_id=(
  SELECT m.home_team_api_id
  FROM Match m
  LEFT JOIN Country c
    ON m.country_id=c.id
    WHERE c.name='Poland' AND m.season='2008/2009'
...
WHERE t.team_api_id=(
  ...
  LEFT JOIN Country c
    ON m.country_id=c.id
    WHERE c.name='{{{country_name}}}' AND m.season='{{{season}}}'
  GROUP BY m.home_team_api_id
  ORDER BY m.home_team_goal DESC
  LIMIT 1
)
